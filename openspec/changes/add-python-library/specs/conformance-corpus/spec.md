...
- **THEN** the corpus stores the input document together with a machine-readable
  parse expectation
- **AND** the corpus stores one or more machine-readable emit expectations
- **AND** each emit expectation can identify the library setting profile it
  applies to
...
- **THEN** it can discover the expected output for each supported setting
  profile from JSON control data
